...
export const booksConnect = {
  async getAllBooks() {},
  async getNewBooks(pageNumber, pageSize = 10, path = "GetBookNew") {
    return await axiosClient.get(`Book/${path}?pageNumber=${pageNumber}&pageSize=${pageSize}`);
  },
  async getMostPopularBooks(pageNumber, pageSize = 10, path = "GetBookByNumberView") {
    return await axiosClient.get(`Book/${path}?pageNumber=${pageNumber}&pageSize=${pageSize}`);
  },
  ...
  async searchBook(pageNumber = 1, pageSize = 12, keyWord) {
    return await axiosClient.get(`Book/SearchBook?pageNumber=${pageNumber}&pageSize=${pageSize}&values=${keyWord}`);
  },
  async getFileImage(id, extensition) {
    return await axiosClient.get(`Book/GetFileImage?fileNameId=${id}.${extensition}`);
  },
