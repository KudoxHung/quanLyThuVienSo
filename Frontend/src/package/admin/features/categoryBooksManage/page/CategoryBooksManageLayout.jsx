...
function _CategoryBooksManageLayout() {
  ...
  useEffect(() => {
    const savedFilters = JSON.parse(
      localStorage.getItem("filtersDanhMucMucSach"),
    );
  ...
  const columns = [
    {
      render: (text, record) => {
        return (
          <Tooltip title="Xem sách">
            <PlusCircleOutlined
              style={{ fontSize: 19, cursor: "pointer" }}
              onClick={(e) => {
                navigate(`/admin/danh-muc-sach/children/${record.id}`, {
                  state: {
        ...
        return (
          <Space size="small">
            <Button
              ...
              onClick={(e) => {
                e.preventDefault();
                navigate(`/admin/danh-muc-sach/edit/${record.id}`);
              }}
              icon={<EditOutlined />}
            >
              Chỉnh sửa
      ...
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Card bordered={false} className="criclebox h-full">
            <Title level={5}>Quản lý danh mục sách</Title>
            <Space
              style={{
                marginBottom: 16,
              }}
            >
              <Button onClick={(e) => navigate("/admin/danh-muc-sach/new")}>
                Thêm mới danh mục
              </Button>
            </Space>
            <Table
...
export const CategoryBooksManageLayout = WithErrorBoundaryCustom(
  _CategoryBooksManageLayout,
);
