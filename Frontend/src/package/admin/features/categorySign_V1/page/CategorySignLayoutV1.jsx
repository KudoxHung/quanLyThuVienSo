...
function _CategorySignLayoutV1() {
  ...
  useEffect(() => {
    document.title = "Danh mục ký hiệu phân loại";
  }, []);
  ...
  useEffect(() => {
    const savedFilters = JSON.parse(localStorage.getItem("filtersKyHieu"));
  ...
  const props = {
    ...
    onChange(info) {
      if (!info || !info.file) {
        message.error("Dữ liệu đầu vào không hợp lệ.");
        return;
      }
      ...
      if (status === "done") {
        ...
        if (response && response.message) {
          ...
          message.error(
            "Lấy biểu mẫu excel danh mục kí hiệu phân loại thất bại.",
          );
        }
      }
    },
        ...
        return (
          <Space size="small">
            <Button
              ...
              onClick={(e) => {
                e.preventDefault();
                navigate(`/admin/ky-hieu/edit/${record.id}`);
              }}
              icon={<EditOutlined />}
              loading={btnLoading}
            >
      ...
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Card bordered={false} className="criclebox h-full">
            <Title level={5}>Quản lý danh mục kí hiệu phân loại</Title>
            <Space
              style={{
                marginBottom: 16,
              }}
            >
              <Button
                onClick={(e) => navigate("/admin/ky-hieu/new")}
                loading={btnLoading}
              >
                Thêm mới danh mục kí hiệu phân loại
              </Button>
...
export const CategorySignLayoutV1 = WithErrorBoundaryCustom(
  _CategorySignLayoutV1,
);
