...
function _NewCategoryDocumentDigitalConnect() {
  ...
  const onFinish = (values) => {
    setBtnLoading(true);
    values.status = 4; // Dùng chung định nghĩa là 4
    documentType
      .create(values)
      .then((res) => {
        ...
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Card bordered={false} className="criclebox h-full">
            <Typography.Title level={3}>
              Thêm mới danh mục tài liệu số dùng chung
            </Typography.Title>
...
export const NewCategoryDocumentDigitalConnect = WithErrorBoundaryCustom(
  _NewCategoryDocumentDigitalConnect,
);
