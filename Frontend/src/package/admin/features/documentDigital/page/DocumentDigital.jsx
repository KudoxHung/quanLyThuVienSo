...
function _DocumentDigital() {
  ...
  const fetchData = (params = {}) => {
    ...
    books
      .GetListDocumentManyParam(
        getRandomuserParams({ ...params, DocumentType: 3 }),
      )
      .then((res) => {
        setBooks(res);
        setPagination({
  ...
  useEffect(() => {
    const savedFilters = JSON.parse(
      localStorage.getItem("filtersKhaiBaoTaiLieu"),
    );
  ...
  const columns = [
    ...
    //   filteredValue: filtersState?.filteredInfo?.Sort || null,
        ...
        return (
          <Space size="small">
            <Button
              ...
              onClick={(e) => {
                e.preventDefault();
                navigate(`/admin/khai-bao-tai-lieu/edit/${record.document.id}`);
              }}
              icon={<EditOutlined />}
            >
              Chỉnh sửa
  ...
  const props = {
    name: "file",
    action: `${apiUrl}/api/Book/InsertDocumentDigital`,
    headers: {
      ...
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Card bordered={false} className="criclebox h-full">
            ...
            >
              <Button
                onClick={(e) => navigate("/admin/khai-bao-tai-lieu/new")}
                loading={btnLoading}
              >
                ...
                onClick={(e) =>
                  books
                    ...
                    .then((res) => {
                      openNotificationWithIcon(
                        "success",
                        "Tải biểu mẫu excel thêm mới tài liệu số thành công",
                        res?.message,
                      );
...
export const DocumentDigital = WithErrorBoundaryCustom(_DocumentDigital);
