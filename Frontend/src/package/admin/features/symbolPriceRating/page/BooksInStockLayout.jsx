...
function _BooksInStockLayout() {
  ...
  useEffect(() => {
    document.title = "Quản lý sách kho lưu trữ";
  }, []);
  ...
  useEffect(() => {
    const fetchData = async () => {
      Promise.all([
        analyst
          .GetNumberDocumentByIdStock(param.id)
          .then((res) => {
            setBooks(res);
            setLoading(false);
          })
  ...
  const columns = [
    ...
    },
    {
      ...
      filters: Books.map((Books) => ({
        text:
          moment(Books.document.publishYear).format("DD/MM/YYYY HH:mm:ss") ===
          "Invalid date"
            ? "Chưa có"
      ...
      render: (_, record) =>
        moment(record.document.publishYear).format("DD/MM/YYYY HH:mm:ss") ===
        "Invalid date"
          ? "Chưa có"
    ...
    },
    {
      ...
      filterSearch: true,
      onFilter: (value, record) =>
        record.document.numberUnlike.startsWith(value),
    ...
    },
    {
      ...
      key: "modifiedDate",
      filters: Books.map((Books) => ({
        text: moment(Books.document.modifiedDate).format("DD/MM/YYYY HH:mm:ss"),
      ...
      <Row gutter={[24, 0]}>
        <Col xs={24} sm={24} md={24} lg={24} xl={24} className="mb-24">
          <Card bordered={false} className="criclebox h-full">
            <Title level={5}>Sách trong kho</Title>
            <Breadcrumb>
...
export const BooksInStockLayout = WithErrorBoundaryCustom(_BooksInStockLayout);
